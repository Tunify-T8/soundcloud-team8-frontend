  ...
  return (
    <div className="min-h-screen bg-zinc-900 text-white">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {/* back + track title */}
        <Link to={`/tracks/${trackId}`} className="flex items-center gap-2 text-zinc-400 hover:text-white text-sm mb-6 transition">
          <ArrowLeft className="w-4 h-4" />
          Back to track
        </Link>

        {/* tabs — matches the design */}
        <div className="flex gap-6 border-b border-zinc-700 mb-8 text-sm">
          {(['likes', 'reposts'] as const).map((tab) => (
            <Link
              key={tab}
              to={tab === 'reposts' ? `/tracks/${trackId}/reposts` : `/tracks/${trackId}/likes`}
        ...
        {loading ? (
          ...
          /* grid of avatars — matches the design */
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6">
            {likes.map((like) => (
              <div key={like.id} className="flex flex-col items-center gap-2 text-center">
                ...
                />
                <p className="text-xs text-zinc-300 font-medium truncate w-full">{like.user.username}</p>
