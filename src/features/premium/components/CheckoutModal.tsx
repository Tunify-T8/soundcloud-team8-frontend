...
export default function CheckoutModal({ plan, onClose }: CheckoutModalProps) {
  ...
  // Profile context to refresh subscription after purchase
  const { setSubscription, token } = useContext(ProfileContext);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await subscribeToplan(payload, token ?? "");

      // Refresh subscription in context immediately
      try {
        const freshSub = await fetchMySubscription(token ?? "");
        setSubscription({
          tier: freshSub.plan as "artist" | "artist-pro" | "free",
          status: freshSub.status,
          data: freshSub,
        });
      } catch {
        // Even if the refresh fails, still show success
      }
  ...
  // Show success modal on top of everything
  if (showSuccess) {
    return (
      <PaymentSuccessModal
        plan={plan}
        ...
        <div className="px-8 pt-10 pb-8">
          ...
          <div className="grid grid-cols-2 gap-10">
                ...
                <div className="space-y-3">
                  ...
                  <label
                    className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${
                      payment === "apple" ? "border-orange-500" : "border-zinc-200 hover:border-zinc-300"
                    }`}
                  >
                    <input
                      type="radio"
                  ...
                  <label
                    className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${
                      payment === "card" ? "border-orange-500" : "border-zinc-200 hover:border-zinc-300"
                    }`}
                  >
                    <input
                      type="radio"
                  ...
                  <label
                    className={`flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${
                      payment === "paypal" ? "border-orange-500" : "border-zinc-200 hover:border-zinc-300"
                    }`}
                  >
                    <input
                      type="radio"
            ...
            <div>
              ...
              {/* Payment failure banner — shown above the button */}
              {paymentError && (
                <PaymentFailedBanner
                  ...
                  {isSubmitting ? (
                    ...
                    <>
                      <span className="font-black italic text-base">
                        ...
                        <span className="text-[#70d0f6]">Pal</span>
                      </span>
