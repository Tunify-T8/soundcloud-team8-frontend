...
function MessageMenu({
  ...
  // Nothing to show for pending messages
    ...
    <div ref={menuRef} className="relative flex items-center">
      <button
        ...
        className="
          ...
          text-zinc-500 transition-opacity duration-150
      ...
      {open && (
        <div
          className={`
            absolute bottom-full mb-1 z-50 min-w-[130px] overflow-hidden
...
function TrackPlayerBubble({
  ...
  return (
    <div
      className={`rounded-2xl ${isMe ? "rounded-br-sm" : "rounded-bl-sm"} px-3 py-3 w-64 ${bubbleBase} transition-opacity ${isPending ? "opacity-60" : "opacity-100"}`}
    >
      ...
      <div className="flex items-center gap-2.5">
        {cover ? (
          ...
          <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded text-lg ${isMe ? "bg-zinc-200" : "bg-zinc-700"}`}>
            🎵
          </div>
        )}
        ...
        <div className="flex-1 min-w-0">
          ...
          {displayArtist && (
            <p className={`text-xs truncate ${subTextColor}`}>{displayArtist}</p>
          )}
        </div>
        ...
        <button
          ...
          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${
            isMe
      ...
      {!isBlocked && !isError && (
        <div className="mt-2.5">
          ...
          >
            <div
              className={`absolute inset-y-0 left-0 rounded-full ${bufferedFill}`}
              style={{ width: `${buffered * 100}%` }}
            />
            <div
              className={`absolute inset-y-0 left-0 rounded-full ${progressFill}`}
...
function PlaylistBubble({
  ...
  return (
    <div
      className={`rounded-2xl ${isMe ? "rounded-br-sm" : "rounded-bl-sm"} px-3 py-2.5 flex items-center gap-3 ${
        isMe ? "bg-white text-zinc-900" : "bg-zinc-800 text-zinc-100"
      } transition-opacity ${isPending ? "opacity-60" : "opacity-100"}`}
    >
      {artworkUrl ? (
        <img src={artworkUrl} alt={title} className="h-10 w-10 shrink-0 rounded object-cover" />
      ) : (
        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded text-lg ${isMe ? "bg-zinc-200" : "bg-zinc-700"}`}>
          🎶
        </div>
      ...
      <div className="min-w-0">
        <p className={`text-[10px] font-bold uppercase tracking-widest leading-none mb-0.5 ${isMe ? "text-zinc-500" : "text-zinc-400"}`}>
          Playlist
        </p>
...
export default function MessageBubble({
  ...
  const artworkUrl =
    (message.attachment?.preview as Record<string, unknown> | null | undefined)
      ...
      <div className={`flex max-w-[75%] flex-col gap-1 ${isMe ? "items-end" : "items-start"}`}>
        ...
        <div className={`flex items-end gap-1.5 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
          {isTrack && attachmentId ? (
            ...
            <PlaylistBubble
              title={message.content ?? "Playlist"}
              artworkUrl={artworkUrl}
              isMe={isMe}
              isPending={isTemp || message.isPending}
            />
