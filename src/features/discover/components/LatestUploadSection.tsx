...
function ActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="flex h-9 w-9 items-center justify-center rounded-[4px] bg-[#f5f5f5] text-[#111] transition-colors hover:bg-white"
    >
      {children}
    </button>
  );
  ...
  return (
    <>
      <section className="mb-12">
        ...
        <div className="rounded-[2px] bg-[#9ba7af] px-5 py-5 sm:px-6 sm:py-6">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 flex-1 flex-col gap-5 sm:flex-row">
              <Link
                to={trackPagePath}
                className="block h-[190px] w-full max-w-[190px] shrink-0 overflow-hidden rounded-[4px] border border-black/10 bg-[#a7b1b8]"
              >
                <img
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                <Link
                  to="/me"
                  className="w-fit text-[14px] font-medium text-white/95 transition-colors hover:text-white"
                >
                  {artistName}
                </Link>
                ...
                <Link
                  to={trackPagePath}
                  className="mt-1 block max-w-[520px] truncate text-[18px] font-bold tracking-tight text-white transition-colors hover:text-white/90 sm:text-[20px]"
                >
                  {track.title}
                </Link>

                <div className="mt-4 flex flex-wrap items-center gap-5 text-[13px] font-semibold text-white">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-5">
                  <div className="flex items-center gap-5">
                    ...
                    <ActionButton
                      ...
                      onClick={() => setShowPlaylistOverlay(true)}
                    >
                  ...
                  <div className="relative" ref={menuRef}>
                    ...
                    >
                      <MoreHorizontal className="h-4 w-4 text-[#ff9d66]" />
                    </ActionButton>

                    {menuOpen ? (
                      <div className="absolute left-0 top-full z-30 mt-3 min-w-[190px] overflow-hidden rounded-[4px] border border-white/10 bg-[#111] py-2 shadow-2xl">
                        <button
                          type="button"
                          onClick={() => {
                            void handleCopyLink();
            ...
            <div className="flex shrink-0 flex-col items-start gap-5 lg:items-end lg:pt-20">
              <Link
                to="/me/insights/overview"
                className="flex items-center gap-1 text-[14px] font-bold text-white transition-colors hover:text-white/80"
              >
                Advanced Insights
              ...
              <Link
                to="/artists"
                className="rounded-[4px] bg-[#2c2c2c] px-4 py-3 text-[14px] font-bold text-white transition-colors hover:bg-[#363636]"
              >
                See all your tracks
              </Link>
            </div>
